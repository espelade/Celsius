...
typedef struct {
  ...
  char str_name1[ SIZE_PLAYERNAME ];//先手
  char str_name2[ SIZE_PLAYERNAME ];//後手
  FILE *pf;// 読み込んでいるCSAファイルのファイルポインタ
  //ゲームの状態,開始局面からの手数,読み込み中のCSAファイルの現在解析している行ナンバー
...
struct tree {
  ...
  short save_material[ PLY_MAX ];// 駒割保存用の配列
