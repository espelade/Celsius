...
#define RazorMargin(depth) (0x10 * depth + 0X200)
  ...
  if ( ! ptree->nsuc_check[ply] )
    {
	  ...
	  if (depth <= PLY_INC
		  && !InCheck(turn)
		  && value + RazorMargin(depth) < beta
		  &&  abs(beta) < score_mate1ply
		  )
	  {
		  int rdepth = 1;
		  int rbeta = beta - RazorMargin(depth);
		  int razorvalue = search(ptree, rbeta - 1, rbeta, turn, rdepth, ply, state_node);
		  if (razorvalue < rbeta)
			{
			  return razorvalue;
			}
	  ...
	  ** End Razoring */
		  ...
		  )
	  {
		  ...
		  if (search(ptree, alpha - margin - 1, alpha - margin, turn, rdepth, ply, state_node) < alpha - margin){ return alpha; }
		  else if (search(ptree, beta + margin, beta + margin + 1, turn, rdepth, ply, state_node) > beta + margin){ return beta; }
	  }
